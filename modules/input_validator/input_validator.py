...
class InputValidator:
    ...
    def validate(self) -> bool:
        try:
            return (self.validate_argv() and 
                    self.validate_files() and 
                    self.validate_sut() and 
                    self.validate_test_vectors_file() and 
                    self.validate_test_vectors_format())
        except:
    ...
    def validate_files(self) -> bool:
        ...
        if not(os.path.isfile(self.__argv[2])):
            print(f"[ERROR] Could not find TestVector input file at path {self.__argv[2]}")
            return False
        return True
    ...
    def validate_test_vectors_format(self) -> bool:
        ...
        if ("Time" in header_row) and ("INPUT_COMMENTS" in header_row) and ("OUTPUT_COMMENTS" in header_row):
            ...
            if time_index < input_comments_index and input_comments_index < output_comments_index:
                # OK, do nothing
                pass
            else:
               print("[ERROR] Bad Test Vector format")
