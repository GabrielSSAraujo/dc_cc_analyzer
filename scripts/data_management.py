...
def script_generator(input_shape, output_shape):
    # Caminho do arquivo .c
    original_file_path = './SUT/teste_driver.c' #TO DO: automatizar a busca do arquivo teste_driver.c
    
    # Caminho do arquivo de backup
    backup_file = './SUT/backup_teste_driver.c'
    # Caminho do arquivo original

    # Copiar o conteúdo do arquivo de backup para o arquivo original
    ...
    conteudo = conteudo.replace('//include SutFileName.h', f'#include "SUT.h"')  #TO DO: automatizar a inclusão do arquivo sut de interesse
    ...
    conteudo = conteudo.replace('//define OUT_COLS', f'#define OUT_COLS {output_shape[1]}')

    # Gerar dinamicamente a declaração de variáveis out
    out_declaration = "int " + ", ".join([f"out{i}" for i in range(output_shape[1])]) + ";" #esse int no inicio deixa fixo que todas as variáveis são inteiras
    ...
    header = generate_header(output_shape[1]) #output_shape[1] é o número de colunas de saída
    conteudo = conteudo.replace("//HeaderPlaceholder", f'fprintf(results_file, "%s,%s\\n",output_line,"{header}");')
    
    # Gerar dinamicamente a chamada da função SUT
    sut_call = "SUT(" + ", ".join([f"input_data[{i}]" for i in range(input_shape[1])]) + ", " + ", ".join([f"&out{i}" for i in range(output_shape[1])]) + ");"
    ...
    conteudo = conteudo.replace("//PassFailComparison", test_result_line) #TO DO: aceitar todos tipos de variavel, não apenas o inteiro (talvez uma struct?)
...
def main(file_path):
    ...
    script_generator(input_shape, output_shape)
    
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Execute: digite o caminho do arquivo até data_management.py e o caminho do arquivo até chegar TestVec_VCP-500-VC-01.xlsx")
        print("Exemplo) Abrindo o terminal em dc_cc_analyzer: python3 ./scripts/data_management.py ./testes/TestVec_VCP-500-VC-01.xlsx")
        sys.exit(1)
...
#comando no python terminal:  python3 ./scripts/data_management.py ./testes/TestVec_VCP-500-VC-01.xlsx
